...
def verify_patient(full_name: str, phone_number: str, date_of_birth: str) -> dict:
    ...
    return {"verified": True, "user_id": result[0], "name": result[1]} if result else {"verified": False}
...
def get_appointments(patient_id: int) -> list:
    ...
    return [{"id": row[0], "date": row[1], "time": row[2], "doctor": row[3], "type": row[4], "status": row[5]} for row in cursor.fetchall()]
