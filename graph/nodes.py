    ...
    try:
        ...
        if extraction_result.data_complete:
            ...
            try:
                ...
                error_response = error_llm.invoke([SystemMessage(content="You are a healthcare assistant. The user provided information but it's not in the correct format. Ask them politely to provide their full name, a 10-digit phone number, and date of birth in YYYY-MM-DD format.")])
                return {
        ...
        try:
            fallback_response = fallback_llm.invoke([SystemMessage(content="You are a healthcare assistant experiencing technical difficulties. Apologize politely and ask the user to try again or provide their information.")])
            return {"messages": [AIMessage(content=fallback_response.message)]}
        except:
            return {"messages": [AIMessage(content="Hello! I'm your healthcare assistant. How can I help you today?")]}
...
def auth_node(state: ChatbotState) -> Dict[str, Any]:
    ...
    if not user_data:
        ...
        response = response_llm.invoke([SystemMessage(content="You are a healthcare assistant. The user needs to provide their personal information (name, phone, date of birth) before you can help them. Ask for this information politely.")])
    ...
    if verification_result["verified"]:
        ...
        try:
            response = llm.invoke([
                ...
                HumanMessage(content="Please welcome me back and ask how you can help.")
            ])
            return {
                "user_verified": True,
            ...
            fallback_llm = llm.with_structured_output(GeneralResponse)
            try:
                welcome_response = fallback_llm.invoke([SystemMessage(content="You are a healthcare assistant. Welcome a verified patient back warmly and ask how you can help them with their appointments.")])
                return {
                    "user_verified": True,
                    "user_data": {**user_data, "user_id": verification_result["user_id"]},
                ...
                return {
                    "user_verified": True,
                    "user_data": {**user_data, "user_id": verification_result["user_id"]},
                    "messages": [AIMessage(content="Welcome back! How can I help you with your appointments today?")]
                }
    else:
        ...
        try:
            response = llm.invoke([
                SystemMessage(content=system_prompt),
                HumanMessage(content="I tried to verify my information but it wasn't found.")
            ])
            ...
            fallback_llm = llm.with_structured_output(GeneralResponse)
            try:
                error_response = fallback_llm.invoke([SystemMessage(content="You are a healthcare assistant. You couldn't verify the user's information in the system. Politely suggest they contact the office for assistance.")])
                return {"user_verified": False, "messages": [AIMessage(content=error_response.message)]}
            except:
                return {"user_verified": False, "messages": [AIMessage(content="I couldn't verify your information. Please contact our office.")]}
...
def chatbot_node(state: ChatbotState) -> Dict[str, Any]:
    ...
    if not state.get("user_verified"):
        ...
        response_llm = llm.with_structured_output(GeneralResponse)
        response = response_llm.invoke([SystemMessage(content="You are a healthcare assistant. The user needs to verify their identity before accessing appointment information. Ask them politely to provide their verification details.")])
        return {"messages": [AIMessage(content=response.message)]}
...
Provide a natural, helpful response in the message field."""
    ...
    try:
        conversation = [SystemMessage(content=system_prompt)] + messages[-4:]  # Recent context
        ...
        fallback_llm = llm.with_structured_output(GeneralResponse)
        try:
            fallback_response = fallback_llm.invoke([SystemMessage(content="You are a healthcare assistant. Ask the user what they'd like to do with their appointments - list, confirm, or cancel them.")])
            return {
                "intent": "end",
            ...
            return {
                "intent": "end",
                "messages": [AIMessage(content="I can help you list, confirm, or cancel appointments. What would you like to do?")]
            }
...
def list_node(state: ChatbotState) -> Dict[str, Any]:
    ...
    if not appointments:
        ...
        apt_info = "\\n".join([f"📅 {apt['type']} with Dr. {apt['doctor']}\\n   📍 {apt['date']} at {apt['time']}\\n   📊 Status: {apt['status'].title()}\\n" for apt in appointments])
...
After listing the appointments, ask if they would like to confirm or cancel any specific appointment, mentioning they can reference by doctor name or appointment type."""
    ...
    try:
        ...
        if appointments:
            ...
            return {
                "available_appointments": appointments,
                "messages": [AIMessage(content=f"Here are your appointments:\\n{apt_text}")]
            }
        else:
            # Generate "no appointments" response using LLM
            no_apt_llm = llm.with_structured_output(GeneralResponse)
            no_apt_response = no_apt_llm.invoke([SystemMessage(content="You are a healthcare assistant. The user has no scheduled appointments. Let them know this in a friendly way and offer to help them schedule one.")])
...
def confirm_node(state: ChatbotState) -> Dict[str, Any]:
    ...
    if not appointments:
        ...
        no_confirm_response = no_confirm_llm.invoke([SystemMessage(content="You are a healthcare assistant. The user doesn't have any appointments to confirm. Let them know this politely and offer to help them schedule an appointment.")])
    ...
    apt_info = "\\n".join([f"ID {apt['id']}: {apt['type']} with {apt['doctor']} on {apt['date']} at {apt['time']} (Status: {apt['status']})" for apt in appointments])
...
Provide a natural, helpful message in all cases."""
    ...
    try:
        ...
        conversation = [SystemMessage(content=system_prompt)] + messages[-6:]  # Include recent conversation
        decision = llm_with_structured_output.invoke(conversation)
        
        if decision.confirm_appointment and decision.appointment_id:
            # Find the appointment in shared state
            apt_to_confirm = next((apt for apt in appointments if apt["id"] == decision.appointment_id), None)
            
            if apt_to_confirm and apt_to_confirm["status"] != "confirmed":
                # Actually confirm the appointment in database
                update_appointment_status.invoke({"appointment_id": decision.appointment_id, "status": "confirmed"})
...
Provide a friendly confirmation message."""
                ...
                try:
                    ...
                    return {
                        "messages": [AIMessage(content=f"✅ Confirmed: {apt_to_confirm['type']} with {apt_to_confirm['doctor']} on {apt_to_confirm['date']}")],
                        "available_appointments": updated_appointments
                    }
            elif apt_to_confirm and apt_to_confirm["status"] == "confirmed":
                return {"messages": [AIMessage(content=decision.message + " This appointment is already confirmed.")]}
            else:
                return {"messages": [AIMessage(content="Sorry, I couldn't find that appointment to confirm.")]}
        else:
        ...
        return {"messages": [AIMessage(content="I'm having trouble processing your confirmation request. Could you please specify which appointment you'd like to confirm?")]}
...
def cancel_node(state: ChatbotState) -> Dict[str, Any]:
    ...
    if not appointments:
        ...
        no_cancel_response = no_cancel_llm.invoke([SystemMessage(content="You are a healthcare assistant. The user doesn't have any appointments to cancel. Let them know this politely and offer to help them schedule an appointment.")])
...
Provide a natural, helpful message in all cases."""
    ...
    try:
        ...
        if decision.cancel_appointment and decision.appointment_id:
            # Find the appointment in shared state
            apt_to_cancel = next((apt for apt in appointments if apt["id"] == decision.appointment_id), None)
            
            if apt_to_cancel and apt_to_cancel["status"] != "cancelled":
                # Actually cancel the appointment in database
                update_appointment_status.invoke({"appointment_id": decision.appointment_id, "status": "cancelled"})
...
Provide a friendly confirmation message."""
                ...
                try:
                    ...
                    return {
                        "messages": [AIMessage(content=f"✅ Your {apt_to_cancel['type']} appointment with {apt_to_cancel['doctor']} on {apt_to_cancel['date']} has been successfully cancelled.")],
                        "available_appointments": updated_appointments  # Update shared state
                    }
                ...
                already_cancelled_response = already_cancelled_llm.invoke([SystemMessage(content="You are a healthcare assistant. The user wants to cancel an appointment that is already cancelled. Let them know this politely.")])
                return {"messages": [AIMessage(content=already_cancelled_response.message)]}
            else:
                # Generate "appointment not found" response using LLM
                not_found_llm = llm.with_structured_output(GeneralResponse)
                not_found_response = not_found_llm.invoke([SystemMessage(content="You are a healthcare assistant. You couldn't find the appointment the user wants to cancel. Ask them to check and try again politely.")])
        ...
        try:
            fallback_llm = llm.with_structured_output(GeneralResponse)
            fallback_response = fallback_llm.invoke([SystemMessage(content="You are a healthcare assistant experiencing technical difficulties with cancellation. Ask the user which appointment they'd like to cancel.")])
            return {"messages": [AIMessage(content=fallback_response.message)]}
        except:
            return {"messages": [AIMessage(content="I can help you cancel an appointment. Which one would you like to cancel?")]}